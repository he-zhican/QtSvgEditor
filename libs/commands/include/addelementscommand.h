...
{
  public:
    AddElementsCommand(std::shared_ptr<SvgDocument> doc, QVector<std::shared_ptr<SvgElement>> elems,
                      QUndoCommand *parent = nullptr)
        : Command("Add Element", parent), m_document(doc), m_elems(elems)
    {
    }

    AddElementsCommand(std::shared_ptr<SvgDocument> doc, std::shared_ptr<SvgElement> elem,
        QUndoCommand* parent = nullptr)
